// 1. Create 4 variables. These variables should store your (1) First Name, (2) Last Name, (3) age, (4) birth month, and (5) Your favorite number. 



// 2. Print each variable to the console



// 3. It's your birthday! Increase your age by one and print the new value out

 

// 4. Decrease your favorite number by 10 and then multiply it by 2. Print out the new value.



// 5. Print out the message that tells the user your name, your age, and your birth month. 



// BONUS: Using innerHTML and document.querySelector(), print the value of your first and last name to the div with an ID of userOutput. Double triple bonus points if you can also make it enclosed between an h2 tag!.